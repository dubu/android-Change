...
public class SimpleTest {
    ...
    @Test
    public void bitBypteIntegerTest() {

    }

    @Test
    public void asdf64괘test() {

        //System.out.println(Changes.P101111.name);
        //System.out.println(Changes.P101111);

        Random rand = new Random();
        int ranNum = rand.nextInt(Changes.values().length);
        //Assert.assertEquals(2,ranNum);

        Assert.assertEquals("sdfsd", Changes.values()[ranNum]);

    }

    @Test
    public void classChangeSmallSlectTest() {
        ...
        Log.info(DubuChanges.values()[upRanNum] + "인데") ;
        Log.info(DubuChanges.values()[downRanNum] + "이다") ;

        //Assert.assertEquals("상괘", Changes.values()[ranNum]);
    }

    @Test
    public void classChangeBigSlectTest() {

        Changes rs = Changes.findName(String.format("%06d", 1100));
        Assert.assertEquals(Changes.P001100, rs);
        Assert.assertEquals(DubuChanges.P100, rs.up);
        Assert.assertEquals(DubuChanges.P001, rs.down);
    ...
    public void 전체의미테스트 (){
        ...
        int unit = 100000;
        for(int i= 0 ; i< 6 ; i++){
            sixUnitNumber = sixUnitNumber + (rand.nextInt(2)*unit) ;
            unit = unit/10;
        }
        ...
        //Changes rs = Changes.findName(String.format("%06d", sixUnitNumber));
        //Assert.assertEquals(rs.name, String.format("%06d", sixUnitNumber));
        //Assert.assertEquals("1110101", String.format("%06d", sixUnitNumber));

        for(int i =0 ; i <64 ; i++){

            String num = Integer.toBinaryString(i);
            Log.info(String.format("%06d", Integer.valueOf(num)));
            Changes rs = Changes.findName(String.format("%06d", Integer.valueOf(num)));
            Assert.assertEquals(rs.name, String.format("%06d", Integer.valueOf(num)));
            Log.info(rs.desc);
        }

        //Assert.assertEquals(Changes.P010110,Changes.findName("010110"));

    }

    @Test
    public void findDubuChanges(){
        DubuChanges rs = DubuChanges.findName("101");
        Assert.assertEquals(DubuChanges.P101, rs);
    }
}
